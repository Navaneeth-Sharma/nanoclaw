...
> **Note:** Commands prefixed with `/` (like `/setup`, `/add-whatsapp`) are [OpenCode skills](https://opencode.ai/docs/en/skills). Type them inside the `opencode` CLI prompt, not in your regular terminal. If you don't have OpenCode installed, get it at [opencode.ai](https://opencode.ai).
...
**Skills over features.** Instead of adding features (e.g. support for Telegram) to the codebase, contributors submit [OpenCode skills](https://opencode.ai/docs/en/skills) like `/add-telegram` that transform your fork. You end up with clean code that does exactly what you need.

**Best harness, best model.** NanoClaw runs on the OpenCode Agent SDK, which means you're running OpenCode directly. OpenCode is highly capable and its coding and problem-solving capabilities allow it to modify and expand NanoClaw and tailor it to each user.

## What It Supports

- **Multi-channel messaging** - Talk to your assistant from WhatsApp, Telegram, Discord, Slack, or Gmail. Add channels with skills like `/add-whatsapp` or `/add-telegram`. Run one or many at the same time.
- **Isolated group context** - Each group has its own `CLAUDE.md` memory (symlinked as `AGENTS.md` for OpenCode), isolated filesystem, and runs in its own container sandbox with only that filesystem mounted to it.
...
**Session Management**
- `/clear` - Add a `/clear` command that compacts the conversation (summarizes context while preserving critical information in the same session). Requires figuring out how to trigger compaction programmatically via the OpenCode Agent SDK.
...
- [Apple Container](https://github.com/apple/container) (macOS) or [Docker](https://docker.com/products/docker-desktop) (macOS/Linux)s
...
```
Channels --> SQLite --> Polling loop --> Container (OpenCode Agent SDK) --> Response
```
...
- `groups/*/CLAUDE.md` - Per-group memory (symlinked as `AGENTS.md` for OpenCode)
